...
class RealisticPortDataGenerator:
    ...
    def _get_time_period_ranges(self, time_periods):
        ...
        if time_periods == 12:  # 每周期2小时
            # 泊位服务时间 1-3小时 → 1-2周期 (实际0.5-1.5周期，向上取整为1-2)
            duration_range = (1, 2)
            # 进港离港时间 1-2小时 → 1周期 (实际0.5-1周期，向上取整为1)
            service_range = (1, 1)  
            # 时间窗 1-3小时 → 1-2周期
            window_range = (1, 2)
            # 系统参数 1小时 → 1周期 (实际0.5周期，向上取整为1)
            prep_time = 1
            tolerance = 1  # 1-2小时取中间值1.5小时 → 1周期
            ...
            # 系统参数
            prep_time = 1  # 1小时 → 1周期
            tolerance = 2  # 1-2小时取中间值1.5小时 → 2周期
            ...
            # 系统参数
            prep_time = 2  # 1小时 → 2周期
            tolerance = 3  # 1-2小时取中间值1.5小时 → 3周期
    ...
    def _generate_realistic_vessels(self, vessel_num, time_periods):
        ...
        for vessel in vessels:
            ...
            if vessel['type'] == 'container':
                if vessel['size'] >= 4:
                    # 大型集装箱船服务时间较长
                    duration = random.randint(duration_range[1], duration_range[1])  # 取上限
                else:
        ...
        for vessel in vessels:
            if vessel['size'] >= 4:
                # 大船需要更多服务时间
                inbound_time = random.randint(service_range[1], service_range[1])  # 取上限
            else:
        ...
        for vessel in vessels:
            if vessel['size'] >= 4:
                # 大船需要更多服务时间
                outbound_time = random.randint(service_range[1], service_range[1])  # 取上限
            else:
    ...
    def _print_realistic_analysis(self, vessel_sizes, berth_capacities, tugboat_capacities, 
        ...
        # 验证时间参数都是整数
        print(f"时间参数验证:")
        print(f"  泊位服务时间范围: {min(vessel_durations)}-{max(vessel_durations)} 周期")
        print(f"  进港服务时间范围: {min(vessel_inbound_times)}-{max(vessel_inbound_times)} 周期")
        print(f"  离港服务时间范围: {min(vessel_outbound_times)}-{max(vessel_outbound_times)} 周期")
    ...
    def generate_realistic_case(self, vessel_num, berth_num, tugboat_num, time_periods=24, 
        ...
        with open(filename, 'w') as f:
            ...
            f.write("# All time parameters are INTEGER multiples of planning periods\n")
            ...
            duration_range, service_range, window_range, _, _ = self._get_time_period_ranges(time_periods)
            f.write(f"# Service durations: {duration_range[0]}-{duration_range[1]} periods ({duration_range[0]*hours_per_period:.1f}-{duration_range[1]*hours_per_period:.1f} hours)\n")
            f.write(f"# Inbound/Outbound times: {service_range[0]}-{service_range[1]} periods ({service_range[0]*hours_per_period:.1f}-{service_range[1]*hours_per_period:.1f} hours)\n")
            f.write(f"# Time windows: {window_range[0]}-{window_range[1]} periods ({window_range[0]*hours_per_period:.1f}-{window_range[1]*hours_per_period:.1f} hours)\n")
            ...
            # 拖船数据
            f.write(f"# Tugboat data ({tugboat_num} tugboats)\n")
            f.write(f"tugboat_capacities = {tugboat_capacities}\n")
            f.write(f"tugboat_costs = {tugboat_costs}\n\n")
            
            # 系统参数
            f.write("# System parameters - All in INTEGER periods\n")
            f.write(f"# Preparation times: {prep_time} periods ({prep_time*hours_per_period:.1f} hours each)\n")
            f.write(f"# Time tolerance: {tolerance} periods ({tolerance*hours_per_period:.1f} hours)\n")
...
if __name__ == "__main__":
    ...
    large_scale_cases = [
        # 50艘船规模 - 小型港口
        (50, 8, 15, 24, "port_50_vessels_T24"),
        (50, 10, 18, 48, "port_50_vessels_T48"),
        
        # 100艘船规模 - 中型港口
        (100, 12, 25, 24, "port_100_vessels_T24"),
        (100, 15, 30, 48, "port_100_vessels_T48"),
        
        # 200艘船规模 - 大型港口
        (200, 20, 40, 24, "port_200_vessels_T24"),
        (200, 25, 50, 48, "port_200_vessels_T48"),
        
        # 300艘船规模 - 特大型港口
        (300, 25, 60, 24, "port_300_vessels_T24"),
        (300, 30, 75, 48, "port_300_vessels_T48"),
        
        # 400艘船规模 - 超大型港口
        (400, 30, 80, 24, "port_400_vessels_T24"),
        (400, 35, 100, 48, "port_400_vessels_T48"),
    ]
    
    print("生成大规模港口调度测试用例...")
    for vessel_num, berth_num, tugboat_num, time_periods, case_name in large_scale_cases:
        print(f"\n正在生成: {case_name}")
